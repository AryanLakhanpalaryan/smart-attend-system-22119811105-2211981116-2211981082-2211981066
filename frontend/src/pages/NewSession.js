...
const NewSession = ({ togglePopup }) => {
  ...
  const createQR = async (e) => {
    ...
    const createSession = async (location) => {
      const formData = {
        token,
        session_id,
        date,
      ...
      try {
        const response = await axios.post("/sessions/create", formData);
        setQrData(response.data.url);
        setQrtoggle(true);
      } catch (err) {
        console.log("Error creating session");
        console.log(err);
        setToastMessage("Error creating session: " + (err?.response?.data?.message || err.message));
      }
    };
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          await createSession(locationString);
        },
        async (error) => {
          console.error("Error getting geolocation:", error);
          await createSession("0,0");
        },
